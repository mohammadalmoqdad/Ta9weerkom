...
$query2=mysqli_query($idd,"select * from user where ID='$c'" );
$id_row2=mysqli_fetch_row($query2); //to seprate the query into several rows.
$c2=$id_row2[1];
...
if(isset($_GET['edit'])) {
	if(mysqli_query($idd,"UPDATE `booking` SET `ID_Status`='3' WHERE `ID` ='$_GET[edit]'")){
		header('location:request.php');
	}
}
...
if(isset($_GET['decline'])) {
	if(mysqli_query($idd,"UPDATE `booking` SET `ID_Status`='1' WHERE `ID` ='$_GET[decline]'")){
		header('location:request.php');
	}
}
...
$sql="SELECT 
booking.id as IDa, booking.Date_Time as date,booking.price as price,booking.description as book_description,
...
where  ID_Photographer='$user' AND ID_Status='2' OR  ID_Status='1'
