...
$query1=mysqli_query($idd,"select * from booking where ID_Client='$user'" );
...
if(isset($_GET['delete'])) {
	if(mysqli_query($idd,"delete from booking where ID = '$_GET[delete]'")){
		header('location:dateclint.php');
	}
}


if(isset($_GET['edit'])) {
	if(mysqli_query($idd,"UPDATE `booking` SET `ID_Status`='4' WHERE `ID` ='$_GET[edit]'")){
...
if(isset($_GET['decline'])) {
	if(mysqli_query($idd,"UPDATE `booking` SET `ID_Status`='3' WHERE `ID` ='$_GET[decline]'")){
		header('location:request.php');
	}
}
...
INNER JOIN user ON booking.ID_Photographer  = user.id
