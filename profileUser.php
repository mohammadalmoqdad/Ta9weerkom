...
$db=mysqli_select_db($idd,"project");
  ...
  $query = mysqli_query($idd,"select * from User where ID ='$ouser'" );
    $id_row=mysqli_fetch_row($query); //to seprate the query into several rows.
    $id=$id_row[0]; //get the wanted row (id row).
    $user_type=$id_row[11];
    $profPic=$id_row[5];
   
    $userRow= mysqli_query($idd,"select * from activity where ID_Mode='1' AND  ID_User ='$ouser' " );
    $fname=$id_row[1];
    $lname=$id_row[2];
    	$username=$fname." ".$lname;


  $query2 = mysqli_query($idd,"select * from activity where ID_User ='$ouser'  AND  ID_Mode='1' " );//return the rows that the photographer have images in .
...
//$wedd=mysqli_query($idd,"select * from price_list  where ID_Photographer='$id' And ID_Subject='$subject' AND ID_Mode='$mode' AND Id_Unit='1' ");
      ...
      <div class="container">
        <div class="row align-items-center">
          ...
          <div class="col-6 col-xl-2">
            <h1 class="mb-0"><a href="home2.php" class="text-black h2 mb-0">TasweerCom<span class="text-primary">.</span></a></h1>
          </div>
       ...
       <div class="row">
         <div class="col-md-12 d-md-flex align-items-center">
           ...
           <div class="ml-auto text-center text-md-left">
            <a <?php echo "href=book.php?id=".$ouser;?> class="btn btn-danger py-3 px-5 rounded">Contact Me</a>
           </div>
         </div>
       </div>
     </div>
